			...
			link : function(scope, element, attrs, ngModel) {
				...
				function setPosition (index) {
					...
					if (curEl && curEl.length) {
						...
						var leftStr = (middle  - (curElLeft) -  (curElWidth/2) - 24);
						//If tabs are not scrollable
						if (!scrollDiv) {
							wrap.style.webkitTransform = "translate3d("+leftStr+"px,0,0)";
						} else {
							//If scrollable tabs
							var wrapWidth = wrap.offsetWidth;
							var transform = (typeof scrollDiv.style.webkitTransform === 'undefined') ? scrollDiv.style.transform : scrollDiv.style.webkitTransform;
							var currentX = Math.abs(getX(transform));
							var leftOffset = 100;
				...
				events.on('slideChange', function(data){
					...
					//console.log(angular.element(window.querySelector(".slider-slide[data-index=" + data.index + "]")));
				});
